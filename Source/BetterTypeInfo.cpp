...
BetterTypeInfo::BetterTypeInfo(std::type_info const* info_, bool is_const_, bool is_ref_, bool is_ptr_)
    : info(info_), is_const(is_const_), is_ref(is_ref_), is_ptr(is_ptr_) {}
...
{
    return TypeInfoPtr(new BetterTypeInfo(info, false, false, false));
}

TypeInfoPtr BetterTypeInfo::as_const_type() const
{
    return TypeInfoPtr(new BetterTypeInfo(info, true, is_ref, is_ptr));
}

TypeInfoPtr BetterTypeInfo::as_nonconst_type() const
{
    return TypeInfoPtr(new BetterTypeInfo(info, false, is_ref, is_ptr));
}

TypeInfoPtr BetterTypeInfo::remove_reference() const
{
    return TypeInfoPtr(new BetterTypeInfo(info, is_const, false, is_ptr));
}
