...
static void display()
{
    ...
    for ( int x=-GRID_W/2; x<GRID_W/2-1; ++x )
    {
        for ( int z=-GRID_H/2; z<GRID_H/2-1; ++z )
        {
            ...
            float a[] = { (float)x+0, grid[x+0+GRID_W/2][z+0+GRID_H/2], (float)z+0 };
            float b[] = { (float)x+0, grid[x+0+GRID_W/2][z+1+GRID_H/2], (float)z+1 };
            float c[] = { (float)x+1, grid[x+1+GRID_W/2][z+1+GRID_H/2], (float)z+1 };
            float d[] = { (float)x+1, grid[x+1+GRID_W/2][z+0+GRID_H/2], (float)z+0 };
